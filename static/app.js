...
function updateDeviceDetail() {
  const id = Number(el.deviceSelect.value);
...
async function startMic() {
  try {
    await api("/api/start", {
      method: "POST",
      body: JSON.stringify({ source: "mic", device_id: Number(el.deviceSelect.value) }),
    });
    await refreshStatus();
  } catch (error) {
    setError(error.message);
...
function handleEvent(event) {
  ...
  if (event.type === "segment") addEvent("segment", `${payload.kind} ${payload.start_ms ?? ""}-${payload.end_ms ?? ""}`);
...
function applyStatus(status) {
  ...
  el.deviceSelect.disabled = state.running;
  el.refreshDevices.disabled = state.running;
  el.startMic.disabled = state.running;
  el.runWav.disabled = state.running;
